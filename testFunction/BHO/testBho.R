...
inputParameter <- read.csv("./testFunction/BHO/inputBHOLampiran.csv", sep = ";")
inputParameter$fungsi <- as.character(inputParameter$fungsi)

test.bho <- function(inputParameter){
  ...
  for(index in 1:nrow(inputParameter)){
    ...
    # testing
    tic("finished in")
    x <- BHO(fungsi, optimType = optimType, numVar = numVar, rangeVar = rangeVar, 
...
result <- test.bho(inputParameter)
write.csv2(result, file = "./testFunction/BHO/resultBHOLampiran.csv")

...
inputParameter <- read.csv("./testFunction/CLONALG/inputClonalgLampiran.csv", sep = ";")
inputParameter$fungsi <- as.character(inputParameter$fungsi)

test.clonalg <- function(inputParameter){
  testClonalgResults <- data.frame()
  for(index in 1:nrow(inputParameter)){
    ...
    selectionSize <- inputParameter$selectionSize[index]
    multipicationFactor <- inputParameter$multipicationFactor[index]
    hypermutationRate <- inputParameter$hypermutationRate[index]
    
    # testing
    tic("finished in")
    x <- CLONALG(fungsi, optimType = optimType, numVar = numVar, rangeVar = rangeVar,
                 maxIter = iterasi, numPopulation = populasi,
                 selectionSize = selectionSize, multipicationFactor = multipicationFactor, hypermutationRate = hypermutationRate)
    time <- toc()
    ...
    testClonalgResult <- data.frame(fungsi, iterasi, populasi, numVar,
                                    selectionSize, multipicationFactor, hypermutationRate,
                                    optimum, x, time)
    testClonalgResults <- rbind(testClonalgResults, testClonalgResult)
  }
  rownames(testClonalgResults) <- 1:nrow(testClonalgResults)
  return(testClonalgResults)
}
result <- test.clonalg(inputParameter)
write.csv2(result, file = "./testFunction/CLONALG/resultClonagLampiran.csv")

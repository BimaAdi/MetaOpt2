...
inputParameter <- read.csv("./testFunction/DE/inputDELampiran.csv", sep = ";")
inputParameter$fungsi <- as.character(inputParameter$fungsi)

test.de <- function(inputParameter){
  ...
  for(index in 1:nrow(inputParameter)){
    ...
    scalingVector <- inputParameter$scalingVector[index]
    crossOverRate <- inputParameter$crossOverRate[index]
    
    # testing
    tic("finished in")
    x <- DE(fungsi, optimType = optimType, numVar = numVar, rangeVar = rangeVar, 
             maxIter = iterasi, numPopulation = populasi,
            scalingVector=scalingVector, crossOverRate=crossOverRate)
    time <- toc()
...
result <- test.de(inputParameter)
write.csv2(result, file = "./testFunction/DE/resultDELampiran.csv")
